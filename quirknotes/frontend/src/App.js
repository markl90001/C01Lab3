...
function App() {
    ...
    try {
      ...
      .then(async (response) => {
        if(!response.ok) {
          ...
          await response.json().then((data) => {
            deleteAllNotesState(data.deletedCount)
          })
        }
      })
    } catch (error) {
  ...
  const deleteNoteState = (_id) => {
    setNotes((prevNotes) => {return prevNotes.filter(note => note._id !== _id)})
    alert(`Note with id ${_id} was deleted`)
  }

  const deleteAllNotesState = (deletedCount) => {
    setNotes([])
    alert(`${deletedCount} notes were deleted`)
  }

  const patchNoteState = (_id, title, content) => {
    setNotes((prevNotes) => {return prevNotes.map((note) => 
      (note._id !== _id) ? note : {_id, title, content})})
  }
